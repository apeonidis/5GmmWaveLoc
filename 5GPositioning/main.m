...
SP=[  8.1716   -6.3877;
    0.9172   -8.3818;
   -0.6265    7.5314];
% rand(L-1,2)*20-10;      % random points uniformly placed in a 20 m x 20 m area 
...
for n=1:N
    for l=1:L
        t1 = h(l)*exp(-1j*2*pi*TOA(l)*(n-1)/(N*Ts));
        t2 = sqrt(Nr)*getResponse(Nr,sin(AOA(l)));
        t3 = sqrt(Nt)*getResponse(Nt,sin(AOD(l)));
        H(:,:,n)=H(:,:,n)+ t1 * (t2* t3'); %% !!! NOTE THAT ' IS CONJUGATE TRANSPOSE!!
    end   
end
...
Hb=Ur'*H(:,:,1)*Ut;
mesh(asin(aa),asin(aa),abs(Hb)); %TODO: Why are two of the parameters asin(aa) ? 
xlabel('AOD'); ylabel('AOA'); zlabel("abs(Hb)");
...
% 
% for i=1:N
%     writematrix(y(:,:,i), sprintf('debug_export_data/exported_y_%d.csv', i));
%     writematrix(F(:,:,i), sprintf('debug_export_data/exported_F_%d.csv', i));
% end

for i=1:N
    y(:,:,i) = readmatrix(sprintf('debug_export_data/exported_y_%d.csv', i));
    F(:,:,i) = readmatrix(sprintf('debug_export_data/exported_F_%d.csv', i));
end
...
yb=zeros(Nr*Ns,N); %!Fixed to "yb" from "ybb"
...
for l=1:L
    distances(l)=-mean(diff(angle(h_hat(l,:))))*(N*Ts)*c/(2*pi);                % simplified from (51)-(54)
    if (distances(l)<0)
